...
function CommandPalette({ children }: { children: React.ReactNode }) {
  ...
  }, [session, connections, router, runCommand, mutate, refetch]);

  const filteredAccountCommands = React.useMemo(() => {
    const searchTerm = searchValue.toLowerCase();
    const searchTerms = ["switch", "account", "switch account", "email"]; // Common search terms
    return accountCommands.filter(
      (command) =>
        // Match title (which contains email)
        command.item.title.toLowerCase().includes(searchTerm) ||
        // Match any of our search terms
        searchTerms.some((term) => term.includes(searchTerm)),
    );
